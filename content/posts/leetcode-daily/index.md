...
- 目標 : 計算道路上車輛碰撞的次數，給定車輛的初始位置和方向

- 關鍵 1 : 使用堆疊來模擬車輛的運動和碰撞過程總碰撞次數 = 「去掉最左邊的一串 'L' 和最右邊的一串 'R' 之後」這個中間區間裡
'L' 和 'R' 的數量（因為每一台會動的車，至少參與一次碰撞）

### 3432. Count Partitions with Even Sum Difference

- 目標 : 計算陣列能被分成兩個子集，使得兩個子集的和差為偶數的分割方式數量

- 關鍵 1 : 陣列總和為偶數時，才能分割成兩個子集使得和差為偶數 
...
- 目標 : 最大化選擇的孩子們的快樂值，給定每個孩子的快樂值和選擇限制

- 關鍵 1 : 使用貪心算法，優先選擇快樂值較高的孩子
